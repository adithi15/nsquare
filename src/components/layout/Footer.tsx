  ...
  return (
    <footer className="relative w-full bg-[#0b0b0b] text-white pt-20 pb-8 overflow-hidden">
      ...
      <div className="relative max-w-7xl mx-auto px-6 md:px-12">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-12 gap-12 lg:gap-10 xl:gap-12">
          {/* Brand */}
          <div className="lg:col-span-4 space-y-6 lg:pr-10">
            <img
              src="/N-Square-logo.png"
              alt="N Square logo"
              className="h-20 md:h-24 w-auto object-contain cursor-pointer hover:opacity-90 transition-opacity"
              onClick={() => {
                if (onSelectTab) {
                  onSelectTab('residences');
                } else {
                  navigate('/');
                }
          ...
          <div className="lg:col-span-3 space-y-6">
            <div className={COLUMN_TITLE} style={{ fontSize: '12px' }}>Help Center</div>
            <div className="space-y-4 text-[14px]">
              <button onClick={() => { navigate('/redevelopment-faq'); window.scrollTo({ top: 0, behavior: 'smooth' }); }} className={LINK_ITEM}>Redevelopment FAQs</button>
              <button onClick={() => { navigate('/privacy-policy'); window.scrollTo({ top: 0, behavior: 'smooth' }); }} className={LINK_ITEM}>Privacy Policy</button>
              <button onClick={() => { navigate('/terms'); window.scrollTo({ top: 0, behavior: 'smooth' }); }} className={LINK_ITEM}>Terms & Conditions</button>
            </div>
          </div>
          ...
          <div className="lg:col-span-3 space-y-6">
            ...
            <div className="space-y-4 text-[14px] text-white/70">
              <a href={CONTACT.phoneHref} className="flex items-center gap-3 hover:text-[#C5A059] transition-colors">
                <PhoneCall className="w-4 h-4 text-[#C5A059] shrink-0" /> {CONTACT.phone}
              </a>
              <a href={CONTACT.whatsappHref} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 hover:text-[#C5A059] transition-colors">
                <MessageCircle className="w-4 h-4 text-[#C5A059] shrink-0" /> WhatsApp Concierge
              </a>
              <a href={`mailto:${CONTACT.email}`} className="flex items-center gap-3 hover:text-[#C5A059] transition-colors">
                <Mail className="w-4 h-4 text-[#C5A059] shrink-0" /> {CONTACT.email}
              </a>
              <div className="flex items-start gap-3 leading-relaxed pt-1">
            ...
            <div className="pt-4 flex items-center gap-4">
              <a href="#" aria-label="Facebook" className="hover:-translate-y-1 transition-transform duration-300">
                <img src="/socials/facebook.png" alt="Facebook" className="w-8 h-8 object-contain opacity-80 hover:opacity-100 transition-opacity" />
              </a>
              <a href="#" aria-label="Instagram" className="hover:-translate-y-1 transition-transform duration-300">
                <img src="/socials/instagram.png" alt="Instagram" className="w-8 h-8 object-contain opacity-80 hover:opacity-100 transition-opacity" />
              </a>
              <a href="#" aria-label="YouTube" className="hover:-translate-y-1 transition-transform duration-300">
                <img src="/socials/youtube.png" alt="YouTube" className="w-8 h-8 object-contain opacity-80 hover:opacity-100 transition-opacity" />
              </a>
              <a href="#" aria-label="LinkedIn" className="hover:-translate-y-1 transition-transform duration-300">
                <img src="/socials/linkedin.png" alt="LinkedIn" className="w-8 h-8 object-contain opacity-80 hover:opacity-100 transition-opacity" />
              </a>
            </div>
          </div>
        </div>
