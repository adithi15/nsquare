        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-6xl mx-auto">
          {REDEVELOPMENT_PROCESS.slice(0, showAllProcess ? REDEVELOPMENT_PROCESS.length : 3).map((item) => (
            ...
            >
              ...
              <span className="absolute top-0 left-0 w-full h-[2px] bg-[#C5A059] transition-transform duration-500 origin-left scale-x-0 group-hover:scale-x-100" />
              {/* Right border (top to bottom) */}
              <span className="absolute top-0 right-0 w-[2px] h-full bg-[#C5A059] transition-transform duration-500 origin-top scale-y-0 group-hover:scale-y-100 delay-[500ms]" />
              {/* Bottom border (right to left) */}
              <span className="absolute bottom-0 right-0 w-full h-[2px] bg-[#C5A059] transition-transform duration-500 origin-right scale-x-0 group-hover:scale-x-100 delay-[1000ms]" />
              {/* Left border (bottom to top) */}
              <span className="absolute bottom-0 left-0 w-[2px] h-full bg-[#C5A059] transition-transform duration-500 origin-bottom scale-y-0 group-hover:scale-y-100 delay-[1500ms]" />

              {/* Giant background number positioned at the top right */}
              <div className="absolute top-0 right-0 m-0 p-0 text-[8.5rem] md:text-[10rem] font-serif font-black text-white dark:text-neutral-800 leading-[0.75] select-none tracking-tighter opacity-80 pointer-events-none transition-transform duration-500 group-hover:scale-105">
                {item.num}
              </div>
