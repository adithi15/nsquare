...
//             className="w-full lg:w-[320px] xl:w-[360px] shrink-0 flex flex-col justify-center items-center lg:items-start text-center lg:text-left relative pb-6 lg:pb-0 lg:ml-8 xl:ml-14 lg:-translate-y-8"
//           >
...
//               className={`w-2.5 h-2.5 rounded-full transition-all duration-500 cursor-pointer ${activeDot === i ? 'bg-white scale-110' : 'bg-[#C5A059]/50 hover:bg-[#C5A059]'
//                 }`}
//             />
//           ))}
//         </div>
...
export const OngoingProjectsCarousel: React.FC<OngoingProjectsCarouselProps> = ({ theme, onViewAll }) => {
  ...
  return (
    <section className="relative py-8 sm:py-10 md:py-12 lg:py-14 bg-[#0C0C0C] overflow-hidden">
      ...
      <div className="relative z-10 px-5 sm:px-8 lg:px-14 xl:px-20">
        <div className="relative flex flex-col lg:flex-row gap-6 lg:gap-8 items-stretch">
          ...
          >
            ...
            <h2 className="font-serif text-2xl sm:text-3xl lg:text-[34px] text-[#C5A059] font-medium leading-[1.2] mb-1.5">
              Signature Projects<br />in the Making
            </h2>
            ...
            <div className="flex gap-4 sm:gap-6 lg:gap-7 items-stretch">
              {ONGOING_PROJECTS.map((p, i) => (
                ...
                >
                  ...
                  <div className="bg-transparent px-4 sm:px-5 py-3 flex items-center justify-between">
                    <div>
                      <h3 className="text-base sm:text-lg text-white leading-snug transition-colors">
                        {p.name}
                      </h3>
          ...
          <motion.button
            ...
            transition={{ duration: 0.9, delay: 0.24, ease: EASE }}
            onClick={onViewAll}
            className="hidden lg:flex shrink-0 flex-row items-center gap-2 cursor-pointer group text-[#C5A059] hover:text-[#B38D48] transition-colors self-center"
          >
            <div className="flex flex-col items-start leading-tight text-left">
              <span className="text-[11px] sm:text-[12px] uppercase tracking-[0.12em] font-medium">
                VIEW ALL
        ...
        <button
          onClick={onViewAll}
          className="lg:hidden mt-5 mx-auto flex items-center gap-3 border border-white/40 text-white px-6 py-2.5 text-[10px] uppercase tracking-[0.3em] font-semibold hover:border-[#C5A059] hover:text-[#C5A059] transition-colors duration-500 cursor-pointer"
        >
          View All Projects <ChevronRight className="w-4 h-4" />
        </button>
      </div>
