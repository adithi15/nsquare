...
// Brochure-style hero titles per tab — white Didone caps + gold script line
const PROJECT_HERO_TITLES: Record<'ongoing' | 'completed' | 'upcoming', { lines: string[]; script: string }> = {
  ongoing: { lines: ['Signature', 'Projects In'], script: 'The Making' },
  completed: { lines: ['Projects That Define', 'Our Legacy'], script: 'Built on Quality and Trust' },
  upcoming: { lines: ['Building Tomorrow', 'Starting Today'], script: 'New Possibilities' },
};
...
export const ProjectsPage: React.FC<ProjectsPageProps> = ({
  ...
  return (
    <div className="w-full bg-[#f4f5f7] min-h-screen pb-24 font-sans">
      
      {/* 1. HERO BANNER SECTION */}
      <section className="relative w-full h-[80vh] min-h-[550px] max-h-[650px] flex flex-col justify-end px-6 md:px-16 pb-40 pt-16 overflow-hidden">
        <div className="absolute inset-0 z-0 overflow-hidden">
          {/* Brochure texture (dark slate + gold shards) — cropped to the text-free band so only the pattern shows */}
          <img
            src="/assets/section-titles/legacy-title.jpg"
            alt=""
            decoding="async"
            className="absolute left-1/2 -translate-x-1/2 top-[-34%] h-[340%] w-auto max-w-none"
          />
          <div className="absolute inset-0 bg-black/45" />
        </div>

        {/* Brochure-style centered title — switches with the active tab */}
        <div className="absolute inset-0 z-10 flex items-center justify-center pb-28 px-4 pointer-events-none">
          <motion.div
            key={activeTab}
          ...
          >
            <h1 className="font-display font-bold uppercase text-white text-4xl sm:text-5xl md:text-6xl leading-[1.2] tracking-wide drop-shadow-[0_2px_14px_rgba(0,0,0,0.75)]">
              {PROJECT_HERO_TITLES[activeTab].lines.map((line) => (
                <span key={line} className="block">{line}</span>
              ))}
            </h1>
            <p className="font-script text-[#d3aa6a] text-3xl sm:text-4xl md:text-5xl mt-2 drop-shadow-[0_2px_10px_rgba(0,0,0,0.6)]">
              {PROJECT_HERO_TITLES[activeTab].script}
            </p>
          </motion.div>
        </div>
